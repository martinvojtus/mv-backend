...
from sqlalchemy import create_engine, Column, Boolean, Integer, String, Text, DateTime, desc
...
class Post(Base):
    ...
    include_timestamps = Column(Boolean, default=True, nullable=False)
...
def update_post(post_id: int, post_update: PostUpdate, db: Session = Depends(get_db), pwd: None = Depends(verify_password)):
    ...
    if post_update.include_timestamps is not None:
